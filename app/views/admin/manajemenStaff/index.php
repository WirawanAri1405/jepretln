             ...
             <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg">
                         ...
                         <div class="w-full md:w-auto flex items-center space-x-3">
                             ...
                             <form id="filterForm" action="" method="GET">
                                 ...
                                 <div id="filterDropdown" class="z-10 hidden w-56 p-3 bg-white rounded-lg shadow dark:bg-gray-700">
                                     <h6 class="mb-3 text-sm font-medium text-gray-900 dark:text-white">Filter berdasarkan Jabatan</h6>
                                     <ul class="space-y-2 text-sm mb-4" aria-labelledby="filterDropdownButton">
                                         <li class="flex items-center">
                                             <input id="role-semua" type="radio" name="roleFilter" value="semua" class="w-4 h-4" <?= ($data['role_aktif'] == 'semua') ? 'checked' : ''; ?>>
                                             <label for="role-semua" class="ml-2 text-sm font-medium">Semua Jabatan</label>
                                         </li>
                                         <?php foreach ($data['roles'] as $role): ?>
                                             <li class="flex items-center">
                                                 <input id="role-<?= $role['id'] ?>" type="radio" name="roleFilter" value="<?= $role['id'] ?>" class="w-4 h-4" <?= ($data['role_aktif'] == $role['id']) ? 'checked' : ''; ?>>
                                                 <label for="role-<?= $role['id'] ?>" class="ml-2 text-sm font-medium"><?= htmlspecialchars($role['display_name']) ?></label>
                                             </li>
                                         <?php endforeach; ?>
                                     </ul>
                             ...
                             <?php else : ?>
                                 <?php foreach ($data['staff'] as $staff) : ?>
                                     <tr class="border-b">
                                         ...
                                         <td class="px-4 py-3 font-medium text-gray-900"><?= htmlspecialchars($staff['name']); ?></td>
                                         ...
                                         <td class="px-4 py-3 flex items-center justify-end">
                                             <button id="staff-dropdown-button-<?= $staff['id']; ?>" data-dropdown-toggle="staff-dropdown-<?= $staff['id']; ?>" class="inline-flex items-center p-0.5 text-sm font-medium text-center text-gray-500 hover:text-gray-800 rounded-lg" type="button">
                                                 <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20">
                                                     <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
                                                 </svg>
                                             </button>
                                             <div id="staff-dropdown-<?= $staff['id']; ?>" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow">
                                                 <ul class="py-1 text-sm" aria-labelledby="staff-dropdown-button-<?= $staff['id']; ?>">
                                                     <li>
                                                         <a href="<?= BASEURL; ?>/Admin/ManajemenStaff/detail/<?= $staff['id']; ?>" class="block py-2 px-4 hover:bg-gray-100">Detail</a>
                                                     </li>
                                                     <li><a href="<?= BASEURL; ?>/Admin/ManajemenStaff/edit/<?= $staff['id']; ?>" class="block py-2 px-4 hover:bg-gray-100">Edit</a></li>
                                                 </ul>
                                                 <div class="py-1">
                                                     <a href="<?= BASEURL; ?>/Admin/ManajemenStaff/hapus/<?= $staff['id']; ?>" class="block py-2 px-4 text-sm text-gray-700 hover:bg-gray-100">Delete</a>
                                                 </div>
                                             </div>
                                         </td>
                                     </tr>
                 ...
                 <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
                     ...
                     <?php
                        ...
                        $queryParams = [];
                        if (!empty($data['role_aktif']) && $data['role_aktif'] !== 'semua') {
                            $queryParams['role'] = $data['role_aktif'];
                        }
                     ...
                     <ul class="inline-flex items-stretch -space-x-px">
                         <li>
                             <?php
                                ...
                                // URL disesuaikan ke ManajemenStaff
                         ...
                         <?php for ($i = 1; $i <= $data['total_pages']; $i++) : ?>
                             <li>
                                 <?php
                                    ...
                                    $pageParams['page'] = $i;
                                    // URL disesuaikan ke ManajemenStaff
                                    $pageHref = BASEURL . '/Admin/ManajemenStaff?' . http_build_query($pageParams);
                                    ?>
                                 <a href="<?= $pageHref ?>"
                         ...
                         <li>
                             <?php
                                ...
                                // URL disesuaikan ke ManajemenStaff
                                $nextHref = ($data['current_page'] < $data['total_pages']) ? BASEURL . '/Admin/ManajemenStaff?' . http_build_query($nextPageParams) : '#';
                                ?>
                             <a href="<?= $nextHref ?>"
